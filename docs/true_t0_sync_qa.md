...
## Q2-1. [CRITICAL] 왜 일반적인 녹음 API(`MediaRecorder`)를 쓰면 안 되나요?
**A.** 모바일 브라우저의 기본 녹음 엔진인 **`MediaRecorder API`는 업로드 용량을 줄이기 위해 내부적으로 Opus 등의 '압축 인코더'를 가동**합니다. 
이 압축 엔진이 시동을 걸고 예열하는 데 소요되는 시간(Encoder Warm-up Latency)이 기종 성능(구형 안드로이드 vs 최신 아이폰)에 따라 **`100ms ~ 300ms`까지 제각각**입니다.
즉, `recorder.start()` 명령을 내린 후, 마이크에서 첫 번째 소리가 실제로 녹음 파일에 기록되기 시작하는 타임스탬프가 기기마다 수천 분의 1초 단위로 완전히 틀어져 버리게 됩니다.

따라서 우리는 불완전한 압축 엔진을 **완전히 폐기**했습니다.
대신, 날것의 오디오 데이터(Raw PCM)를 하드웨어 칩셋에서 직접 뽑아올리는 **`WebAudio API - ScriptProcessorNode` 아키텍처**를 모든 녹음 모드(Studio, Part, Solo 등)에 강제로 적용했습니다.
마이크가 데이터를 토해내는(yield) 정확히 **"첫 번째 0.00ms의 프레임(Byte)"**을 감지하는 순간(`.onaudioprocess`), 그 물리적인 찰나의 이벤트에 맞춰 스피커로 오디오 반주(MR)를 강제 점화(Trigger)시킴으로써, 압축 지연이나 네트워크 버퍼 연산 없이 **"녹음된 목소리의 0.00초"와 "MR의 0.00초"가 태생부터 수학적으로 100% 일치하도록 용접(Lock)**한 것이 핵심 특허 기술입니다.
