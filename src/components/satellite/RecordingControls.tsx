...
export function RecordingControls({
    ...
    return (
        <div className="w-full flex-shrink-0 pt-2 flex flex-col gap-2 relative z-20">
            {/* 1. Dynamic Recording Profile Toggle */}
            <div className="w-full space-y-1.5 mb-1 p-3 bg-slate-900/80 backdrop-blur-md rounded-2xl border border-white/5">
                <label className="block text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">마이크 수음 범위 실시간 변경 (Gain)</label>
                <div className="flex bg-black/40 rounded-2xl overflow-hidden border border-white/5 w-full p-1">
                    <button
                        ...
                        className={`flex-1 py-2 sm:py-3 px-1 text-[11px] sm:text-xs font-bold rounded-xl transition-all ${recordingProfile === 'part' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 py-2 sm:py-3 px-1 text-[11px] sm:text-xs font-bold rounded-xl transition-all ${recordingProfile === 'quartet' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 py-2 sm:py-3 px-1 text-[11px] sm:text-xs font-bold rounded-xl transition-all ${recordingProfile === 'solo' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                    >
            ...
            {(isMrReady && !isRecording) && (
                <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 sm:gap-3 py-3 sm:py-4 font-bold rounded-2xl transition-all shadow-lg active:scale-[0.98] ${isSoloRecording
                ...
                >
                    {isSoloRecording ? (
                        ...
                        <>
                            <Play size={18} className="fill-white/80 ml-1 sm:w-5 sm:h-5" />
                            <span className="text-base sm:text-lg">혼자 연습 (MR 재생)</span>
                        </>
                    )}
                </button>
            )}
