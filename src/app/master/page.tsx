...
export default function MasterPage() {
    ...
    const { status: wsStatus, satellites, connect, disconnect } = useMasterSubscriber(roomId);
    ...
    return (
        <main className="min-h-[100dvh] bg-slate-950 text-slate-100 flex flex-col pt-safe-top pb-safe-bottom">
            {/* Header */}
            <header className="px-6 py-4 flex items-center justify-between border-b border-slate-800 bg-slate-900/50 backdrop-blur-md sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 text-indigo-400 font-bold bg-indigo-500/10 px-3 py-1.5 rounded-lg">
                ...
                {satelliteArray.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 auto-rows-fr">
                        {satelliteArray.map((sat) => {
                            ...
                            return (
                                <div key={sat.part} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col relative overflow-hidden transition-all duration-200 hover:border-slate-700 hover:shadow-xl shadow-none">
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center mb-6">
                                        {p ? (
                                            ...
                                            <div className="text-slate-600 font-medium">수음 대기 중...</div>
                                        )}
                                    </div>
